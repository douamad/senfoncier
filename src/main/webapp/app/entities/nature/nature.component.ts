...
export class NatureComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.natureService
      ...
      .subscribe(
        (res: HttpResponse<INature[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: INature): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInNatures(): void {
    this.eventSubscriber = this.eventManager.subscribe('natureListModification', () => this.loadPage());
  }
