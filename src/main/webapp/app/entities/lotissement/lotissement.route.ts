import { Injectable } from '@angular/core';
import { HttpResponse } from '@angular/common/http';
import { Resolve, ActivatedRouteSnapshot, Routes, Router } from '@angular/router';
import { Observable, of, EMPTY } from 'rxjs';
import { flatMap } from 'rxjs/operators';

import { Authority } from 'app/shared/constants/authority.constants';
import { UserRouteAccessService } from 'app/core/auth/user-route-access-service';
import { ILotissement, Lotissement } from 'app/shared/model/lotissement.model';
import { LotissementService } from './lotissement.service';
import { LotissementComponent } from './lotissement.component';
import { LotissementDetailComponent } from './lotissement-detail.component';
import { LotissementUpdateComponent } from './lotissement-update.component';

@Injectable({ providedIn: 'root' })
export class LotissementResolve implements Resolve<ILotissement> {
  constructor(private service: LotissementService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ILotissement> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((lotissement: HttpResponse<Lotissement>) => {
          if (lotissement.body) {
            return of(lotissement.body);
          } else {
            this.router.navigate(['404']);
            return EMPTY;
          }
        })
      );
    }
    return of(new Lotissement());
  }
}

export const lotissementRoute: Routes = [
  {
    path: '',
    component: LotissementComponent,
    data: {
      authorities: [Authority.USER],
      defaultSort: 'id,asc',
      pageTitle: 'Lotissements',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: ':id/view',
    component: LotissementDetailComponent,
    resolve: {
      lotissement: LotissementResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'Lotissements',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: 'new',
    component: LotissementUpdateComponent,
    resolve: {
      lotissement: LotissementResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'Lotissements',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: ':id/edit',
    component: LotissementUpdateComponent,
    resolve: {
      lotissement: LotissementResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'Lotissements',
    },
    canActivate: [UserRouteAccessService],
  },
];
