...
public class ActiviteResourceIT {
    ...
    public static Activite createEntity(EntityManager em) {
        Activite activite = new Activite().code(DEFAULT_CODE).libelle(DEFAULT_LIBELLE);
        return activite;
    }
    ...
    public static Activite createUpdatedEntity(EntityManager em) {
        Activite activite = new Activite().code(UPDATED_CODE).libelle(UPDATED_LIBELLE);
        return activite;
    }
    ...
    public void createActivite() throws Exception {
        ...
        restActiviteMockMvc
            .perform(post("/api/activites").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(activiteDTO)))
    ...
    public void getAllActivites() throws Exception {
        ...
        restActiviteMockMvc
            .perform(get("/api/activites?sort=id,desc"))
    ...
    public void getActivite() throws Exception {
        ...
        // Get the activite
        restActiviteMockMvc
            .perform(get("/api/activites/{id}", activite.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    public void getNonExistingActivite() throws Exception {
        // Get the activite
        restActiviteMockMvc.perform(get("/api/activites/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
    }
    ...
    public void updateActivite() throws Exception {
        ...
        updatedActivite.code(UPDATED_CODE).libelle(UPDATED_LIBELLE);
        ActiviteDTO activiteDTO = activiteMapper.toDto(updatedActivite);

        restActiviteMockMvc
            .perform(put("/api/activites").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(activiteDTO)))
            .andExpect(status().isOk());
    ...
    public void deleteActivite() throws Exception {
        ...
        // Delete the activite
        restActiviteMockMvc
            .perform(delete("/api/activites/{id}", activite.getId()).accept(MediaType.APPLICATION_JSON))
