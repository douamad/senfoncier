...
public class UsageResourceIT {
    ...
    public static Usage createEntity(EntityManager em) {
        Usage usage = new Usage().code(DEFAULT_CODE).libelle(DEFAULT_LIBELLE);
        return usage;
    }
    ...
    public static Usage createUpdatedEntity(EntityManager em) {
        Usage usage = new Usage().code(UPDATED_CODE).libelle(UPDATED_LIBELLE);
        return usage;
    }
    ...
    public void createUsage() throws Exception {
        ...
        restUsageMockMvc
            .perform(post("/api/usages").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(usageDTO)))
    ...
    public void getAllUsages() throws Exception {
        ...
        restUsageMockMvc
            .perform(get("/api/usages?sort=id,desc"))
    ...
    public void getUsage() throws Exception {
        ...
        // Get the usage
        restUsageMockMvc
            .perform(get("/api/usages/{id}", usage.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    public void getNonExistingUsage() throws Exception {
        // Get the usage
        restUsageMockMvc.perform(get("/api/usages/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
    }
    ...
    public void updateUsage() throws Exception {
        ...
        updatedUsage.code(UPDATED_CODE).libelle(UPDATED_LIBELLE);
        UsageDTO usageDTO = usageMapper.toDto(updatedUsage);

        restUsageMockMvc
            .perform(put("/api/usages").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(usageDTO)))
            .andExpect(status().isOk());
    ...
    public void deleteUsage() throws Exception {
        ...
        // Delete the usage
        restUsageMockMvc
            .perform(delete("/api/usages/{id}", usage.getId()).accept(MediaType.APPLICATION_JSON))
